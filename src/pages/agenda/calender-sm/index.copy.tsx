...
function CalenderSm() {
  ...
  // >([]);
  const loadLastMonth = (key: number) => {
    console.log("logged");
    setCalendars((r) => {
      return [
        renderCalendar(rewindDate.clone().subtract(key, "month").toDate()),
      ].concat(r);
    });
    // }
    // setIsLoading(true);
  ...
  //   console.log("heelo");
  //   if (directionDown) {
  //     Math.abs(diffScroll) > 10 && calendars.length < 10 && loadNextMonth();
  //   } else {
  //     Math.abs(diffScroll) > 10 && calendars.length < 10 && loadLastMonth();
  //   }
  // };
  useEffect(() => {
    // for (let i = 0; i < 10; i++) {
    ...
    >
      ...
      >
        ...
        {/* <VirtualList
          width="100%"
          height={"100%"}
          itemCount={nextCalendars.length}
          ...
          <div>
            {r.daysArray.map((i) => (
              <p className="text-white">
                {moment({ day: i, month: r.month, year: r.year }).format(
                  "DD:MM:YYYY"
                )}
              </p>
            ))} */}
